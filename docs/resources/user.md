...
```hcl
# MySQL, MariaDB, and TiDB 8.5.7+: set MAX_USER_CONNECTIONS
...
* `max_user_connections` - (Optional) Maximum number of simultaneous connections for the user. A value of `0` means unlimited. Supported on MySQL, MariaDB, and TiDB 8.5.7 or newer. Earlier TiDB 8.5.x builds can accept the syntax without consistently exposing usable persisted/readback behavior, so provider support is intentionally gated to TiDB 8.5.7+. When this argument is removed from configuration, the limit is reset to `0`.
